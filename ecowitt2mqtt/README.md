...
# ecowitt2mqtt mods by jumpinf00l
ecowitt2mqtt is an amazing piece of work, and this fork changes the way some features work to suit my unique preferences.

## Differences vs. original code

- Consolidated into a single Github repository. The final container is now built locally by Home Assistant without a pre-staged/build container
- Aligned versioning of ecowitt2mqtt Python app with container/releases for consistency
- Added 'illuminance' entity as a measure of visible light energy calculated from the 'solarradiation' value
  - Uses "lx" by default, or will convert to the unit defined in 'output_unit_illuminance' if defined
  - Note: Home Assistant only supports illuminance values in "lx" and will throw warnings if any other unit is used
- Replaced the 'solarradiation' entity with 'irradiance'
  - 'irradiance' uses the 'irradiance' device class instead of 'illuminance'
  - 'irradiance' uses "W/m²" regardless of the value of 'output_unit_illuminance'
- Replaced the 'solarradiation_perception' entity with 'illuminance_perception'
  - 'illuminance_perception' is a percentage multiplier applied to the 'illuminance' entity value which results in a real-world representation of the human perception of light energy
- Added 'illuminance_perceived' entity as a calculated value of "illuminance" x ("illuminance_perception" / 100)
  - 'illuminance_perceived' is a real-world representation of the human perception of light energy
  - 'illuminance_perceived' reports in the user's selected 'output_unit_illuminance' ("lx" by default for Home Assistant compatibility)
- The Home Assistant App (formerly Add-On) will be shut down to perform cold backups
  - If you perform scheduled backups, the App will shut down during these backups and will start back up once the backup is complete
  - This avoids accidentally backing up session data

## ⚠️ Breaking changes vs. original code ⚠️
- The 'solarradiation_perception' and 'solarradiation' entities will be removed from Home Assistant
  - <b>Historic data will not be migrated. You will lose historic data for these entites</b>
  - The 'solarradiation_perception' entity is replaced with 'illuminance_perception'
  - The 'solarradiation' entity is replaced with 'irradiance'
  - The entities be nullified in MQTT as a tombstone which Home Assistant happily ignores, so removes the entities from the device
- The 'irradiance' entity (replaces the 'solarradiation' entity) now uses the 'irradiance' device class and will always report in "W/m²" regardless of the user's selected 'output_unit_illuminance'
  - The 'illuminance' entity reports the illuminance value in the user's selected 'output_unit_illuminance'
- This app is intended for use as a Home Assistant App only
  - Not tested as a traditional Docker container
  - May not run as expected or at all outside of Home Assistant

## General notes
- Home Assistant only supports the "lx" unit of measure for reporting illuminance
  - If using with Home Assistant, leave 'output_unit_illuminance' set to "lx" otherwise Home Assistant will log a warning
- The "W/m²" unit of measure remains available as an option for 'output_unit_illuminance', but changing this value is not recommended (see above)

## Installation
### Via links
1. Add the app repository to your Home Assistant App Store (click the 'Add' button):

   [![Open your Home Assistant instance and show the add app repository dialog with a specific repository URL pre-filled.](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Fjumpinf00l%2Fecowitt2mqtt)

2. Install the app from your Home Assistant App Store:

   [![Open your Home Assistant instance and show the dashboard of an app.](https://my.home-assistant.io/badges/supervisor_addon.svg)](https://my.home-assistant.io/redirect/supervisor_addon/?addon=cfe19cc8_ecowitt2mqtt&repository_url=https%3A%2F%2Fgithub.com%2Fjumpinf00l%2Fecowitt2mqtt)

3. Configure the App options via the Configuration tab

### Manually
1. Browse to Settings > Apps > Install app (bottom-right) > Three dots (top-right)
2. Enter the following repository: https://github.com/jumpinf00l/ecowitt2mqtt
3. Click Add
4. Click Close
5. Click ecowitt2mqtt
6. Click Install
3. Configure the App options via the Configuration tab

***

# Original work by bachya
> [!NOTE]
> Links below continue to direct to bachya's original work, please support them. Image URLs have been modified to work with this repo.
...
[awnet-upload-screen]: https://raw.githubusercontent.com/jumpinf00l/ecowitt2mqtt/main/ecowitt2mqtt/resources/awnet-upload-screen.jpeg?raw=true
...
[logo]: https://raw.githubusercontent.com/jumpinf00l/ecowitt2mqtt/main/ecowitt2mqtt/resources/logo-full.png
...
[ws-view-upload-screen]: https://raw.githubusercontent.com/jumpinf00l/ecowitt2mqtt/main/ecowitt2mqtt/resources/ws-view-upload-screen.jpeg?raw=true
