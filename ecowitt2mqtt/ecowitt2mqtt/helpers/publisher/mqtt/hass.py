...
from ecowitt2mqtt.const import (
    ...
    DATA_POINT_ILLUMINANCE,
    DATA_POINT_ILLUMINANCE_PERCEPTION,
    DATA_POINT_ILLUMINANCE_PERCEIVED,
    DATA_POINT_INTERVAL,
...
class EntityDescription:
    ...
    Note: object_id is part of the MQTT Topic and not a deprecated Home Assistant object_id.
    """
...
# Dictionary containing deprecated sensor entities to be tombstoned on first run
DEPRECATED_ENTITIES = {
    DATA_POINT_SOLARRADIATION_PERCEIVED: {
        "platform": "sensor"
    },
...
ENTITY_DESCRIPTIONS = {
    ...
    DATA_POINT_ILLUMINANCE: EntityDescription(
        device_class=DeviceClass.ILLUMINANCE,
        state_class=StateClass.MEASUREMENT,
    ),
    DATA_POINT_ILLUMINANCE_PERCEPTION: EntityDescription(
        icon="mdi:brightness-5",
        state_class=StateClass.MEASUREMENT,
    ),
    DATA_POINT_ILLUMINANCE_PERCEIVED: EntityDescription(
        icon="mdi:brightness-5",
        state_class=StateClass.MEASUREMENT,
    ),
...
class HomeAssistantDiscoveryPublisher(MqttPublisher):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, config: Config, client: Client) -> None:
        ...
        self._purged_entities: set[str] = set() # Recently purged deprecated entities
    ...
    def _get_discovery_info(
        ...
        description = ENTITY_DESCRIPTIONS.get(data_point_key)

        topic_key = (
            description.object_id 
            if description and description.object_id 
            else payload_key
        )

        base_topic = (
            f"{self._config.hass_discovery_prefix}/{PLATFORM_MAP[data_point.data_type]}"
            f"/{device.unique_id}/{topic_key}"
        )
        ...
        discovery = HassDiscoveryInfo(
            ...
            unique_id=f"{device.unique_id}_{topic_key}",
        )

        # Maintain original object_id prefix logic
        if self._config.hass_entity_id_prefix:
            discovery.object_id = f"{self._config.hass_entity_id_prefix}_{topic_key}"
        if data_point.unit:
            discovery.unit_of_measurement = data_point.unit

        # Apply remaining entity description attributes
        if description:
    ...
    async def async_publish(self, data: dict[str, CalculatedValueType]) -> None:
        ...
        # Tombstone deprecated entities on first run
        for entity, details in DEPRECATED_ENTITIES.items():
            platform = details["platform"]
            purge_id = f"{processed_data.device.unique_id}_{entity}"

            if purge_id not in self._purged_entities:
                base_topic = (
                    f"{self._config.hass_discovery_prefix}/{platform}/{processed_data.device.unique_id}/{entity}"
                )
                for suffix in ("config", "availability", "attributes", "state"):
                    tasks.append(
                        asyncio.create_task(
                ...
                self._purged_entities.add(purge_id)
                LOGGER.debug(
                    "Tombstone placed for deprecated entity %s", purge_id
                )
